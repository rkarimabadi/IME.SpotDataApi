...
namespace IME.SpotDataApi.Services.CommodityLevel
{
    public class CommodityService : ICommodityService
    {
        ...
        public CommodityService(IDbContextFactory<AppDataContext> contextFactory, IDateHelper dateHelper, ICacheService cacheService)
        {
        ...
        {
            string cacheKey = $"Commodity_Header_{commodityId}";
        ...
        {
            string cacheKey = $"Commodity_Hierarchy_{commodityId}";
            return await _cacheService.GetOrCreateAsync(cacheKey, async () =>
            {
                using var context = await _contextFactory.CreateDbContextAsync();
        ...
        {
            string cacheKey = $"Commodity_PriceTrends_{commodityId}";
            return await _cacheService.GetOrCreateAsync(cacheKey, async () =>
            {
                using var context = await _contextFactory.CreateDbContextAsync();
        ...
        {
            string cacheKey = $"Commodity_MarketAbsorption_{commodityId}";
            return await _cacheService.GetOrCreateAsync(cacheKey, async () =>
            {
                using var context = await _contextFactory.CreateDbContextAsync();
        ...
        {
            string cacheKey = $"Commodity_Attributes_{commodityId}";
            return await _cacheService.GetOrCreateAsync(cacheKey, async () =>
            {
                using var context = await _contextFactory.CreateDbContextAsync();
                ...
                items.Add(new CommodityAttributeItem
                {
                    ...
                    CurrentValue = $"{latestOffer.PrepaymentPercent}%",
                    ...
                    CommodityMostCommonValue = $"{commonCommodityPrepayment}%",
                    MarketMostCommonValue = $"{commonMarketPrepayment}%"
                });
                ...
                items.Add(new CommodityAttributeItem
                {
                    ...
                    CurrentValue = currentMeasurementUnit,
                    ...
                    CommodityMostCommonValue = commonCommodityMeasurementUnit.Name,
                    MarketMostCommonValue = commonMarketMeasurementUnit.Name
                });

                // 6. Tick Size
                items.Add(new CommodityAttributeItem
                {
                    Title = "حداقل تغییر قیمت",
                    CurrentValue = $"{latestOffer.TickSize.ToString("N0")} {currentCurrency}",
                    IconCssClass = "bi bi-graph-up",
                    ...
                    CommodityMostCommonValue = $"{commonCommodityTickSize.ToString("N0")} {commonCommodityCurrency}",
                    MarketMostCommonValue = $"{commonMarketTickSize.ToString("N0")} {commonMarketCurrency}"
                });
                ...
                items.Add(new CommodityAttributeItem
                {
                    ...
                    CommodityMostCommonValue = commonCommodityCurrency.Name,
                    MarketMostCommonValue = commonMarketCurrency.Name
                });
                ...
                {
                    Title = "ضریب وزنی",
                    CurrentValue = $"{latestOffer.WeightFactor:N0} {currentMeasurementUnit}",
                    ...
                    CommodityMostCommonValue = $"{commonCommodityWeightFactor:N0} {commonCommodityMeasurementUnit}",
                    MarketMostCommonValue = $"{commonMarketWeightFactor:N0} {commonMarketMeasurementUnit}"
                });
        ...
        {
            string cacheKey = $"Commodity_MainPlayers_{commodityId}";
            return await _cacheService.GetOrCreateAsync(cacheKey, async () =>
            {
                using var context = await _contextFactory.CreateDbContextAsync();
                var thirtyDaysAgo = _dateHelper.GetPersian(DateTime.Now.AddDays(-30));
        ...
        {
            string cacheKey = $"Commodity_DistributedAttributes_{commodityId}";
            return await _cacheService.GetOrCreateAsync(cacheKey, async () =>
            {
                using var context = await _contextFactory.CreateDbContextAsync();
                var thirtyDaysAgo = _dateHelper.GetPersian(DateTime.Now.AddDays(-30));
        ...
        {
            string cacheKey = $"Commodity_OfferHistory_{commodityId}";
            return await _cacheService.GetOrCreateAsync(cacheKey, async () =>
        ...
        {
            string cacheKey = $"Commodity_PlayerDistribution_{commodityId}";
            return await _cacheService.GetOrCreateAsync(cacheKey, async () =>
            {
                using var context = await _contextFactory.CreateDbContextAsync();
                var thirtyDaysAgo = _dateHelper.GetPersian(DateTime.Now.AddDays(-30));
